...
def remove_fields(product):
  ...
  for i in sales_data_index:
    if product[i]<0:
      return 0
  if product[2]==0 or product[3]>1000:
    return 0
  return product
...
def get_highest_sales(products):
  highest_sales=0
  for product in products:
    if product[3]>highest_sales:
      highest_sales=product[3]
...
def open_file_and_work(file_name):
  ...
  try:
    ...
    new_filename=create_new_file(cleaned_data.insert(0,header),file_name)
